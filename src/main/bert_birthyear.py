...
from sklearn.metrics import f1_score
from constants import MAX_LEN, BATCH_SIZE, EPOCH_SIZE, BIRTHYEAR_SIZE
...
#get birthyear
def get_birthyear(df):
    years=df[1].values
    years=[int(year) for year in years]
    return years

labels = get_birthyear(df)
...
model = BertForSequenceClassification.from_pretrained("bert-base-uncased", num_labels=BIRTHYEAR_SIZE)
#model.cuda()
...
def evaluate_data_for_epoch():
    ...
    for batch in validation_dataloader:
        ...
        evaluateBirthyearPredictions(label_ids, predicted_data)
