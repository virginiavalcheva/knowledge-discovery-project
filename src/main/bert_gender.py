...
from sklearn.metrics import f1_score
from constants import MAX_LEN, BATCH_SIZE, EPOCH_SIZE, GENDER_SIZE
...
def get_gender(df):
    genders=df[2].values
    genders=[int(gender) for gender in genders]
    return genders

labels = get_gender(df)
...
model = BertForSequenceClassification.from_pretrained("bert-base-uncased", num_labels=GENDER_SIZE)
#model.cuda()
...
def evaluate_data_for_epoch():
    ...
    for batch in validation_dataloader:
        ...
        evaluateGenderPredictions(label_ids, predicted_data)
